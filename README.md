...
项目需要两个**免费** API Key。部署到 GitHub Pages 时 Key 已内置在构建中；本地开发时需自行配置：
...
2. 推送代码 → 自动触发构建 → 部署到 `https://xfki.github.io/Global-monitor/`

> API Key 已内置在 `.github/workflows/deploy.yml` 的 `env:` 块中，无需额外配置。
...
- **CORS 代理级联**：`allorigins.win`（主）→ `corsproxy.io`（备）→ 直连（支持 CORS 的 API）
- **i18n**：中英文切换（`localStorage` 持久化，无外部依赖）
